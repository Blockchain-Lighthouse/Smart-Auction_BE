import { BadRequestException, Body, Controller, Get, Param, Post, Req, Request, UnauthorizedException, UseGuards } from '@nestjs/common';
...
export class AuctionController {
    ...
    async getMyFavorites(@Req() req) {
        return this.auctionService.getMyFavorites(req.user.id);
    }
    ...
    async createAution(@Body() body : CreateAuctionDto, @Request() req) {
        return this.auctionService.createAuction(body, req.user.id);
    }
    ...
    async getAuctions(@Param('page') page, @Param('limit') limit) {
        return this.auctionService.getAuctions(Number(page), Number(limit));
    }
    ...
    async searchAuction(@Param("word") word, @Param('page') page, @Param('limit') limit) {
        return this.auctionService.searchAuction(word, Number(page), Number(limit));
    }
    ...
    async bid(@Body() body : BidDto, @Request() req) {
        return this.auctionService.bid(body, req.user.id);
    }
    ...
    async getAuctionRecord(@Param("id") auctionId) {
        return this.auctionService.getAuctionRecord(Number(auctionId));
    }
    ...
    async getAuctionBidders(@Body() body : BidderDto) {
        return this.auctionService.getBidders(body.contractAdrs)
    }
    ...
    async withdrawBySeller(@Body() body : WithdarwDto, @Request() req) {
        return this.auctionService.withdrawBySeller(body, req.user.id);
    }
    ...
    async emergencyWithdraw(@Body() body : WithdarwDto, @Request() req) {
        return this.auctionService.emergencyWithdraw(body, req.user.id);
    }
    ...
    async handleFavorite(@Body() body : FavoriteDto, @Request() req) {
        return this.auctionService.handleFavorites(body.auctionId, req.user.id);
    }
}
